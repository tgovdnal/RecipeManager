...
const recipeSchema = z.object({
  ...
  ingredients: z.array(z.object({ value: z.string().min(2, "Zutat darf nicht leer sein") })).min(1, "Mindestens eine Zutat erforderlich"),
  instructions: z.array(z.object({ value: z.string().min(2, "Schritt darf nicht leer sein") })).min(1, "Mindestens ein Schritt erforderlich"),
  tags: z.string(),
...
export default function RecipeForm({ initialData }: RecipeFormProps) {
  ...
  if (initialData) {
    try {
      ...
      if (Array.isArray(parsedIngs) && parsedIngs.length > 0) {
        ...
        defaultIngredients = initialData.ingredients.split('\n').filter(i => i.trim()).map(i => ({ value: i }));
      }
    } catch (e) {
      defaultIngredients = initialData.ingredients.split('\n').filter(i => i.trim()).map(i => ({ value: i }));
    }
    ...
    try {
      ...
      if (Array.isArray(parsedInsts) && parsedInsts.length > 0) {
        defaultInstructions = parsedInsts.map((inst: string) => ({ value: inst }));
      } else {
        defaultInstructions = initialData.instructions.split('\n').filter(i => i.trim()).map(i => ({ value: i }));
      }
    } catch (e) {
      defaultInstructions = initialData.instructions.split('\n').filter(i => i.trim()).map(i => ({ value: i }));
    }
  ...
  const { register, control, handleSubmit, formState: { errors, isSubmitting } } = useForm<RecipeFormData>({
    ...
    defaultValues: {
      ...
      difficulty: (initialData?.difficulty as 'Einfach' | 'Mittel' | 'Schwer') || "Einfach",
  ...
  const onSubmit = async (data: RecipeFormData) => {
    ...
    const ingredientsJson = JSON.stringify(data.ingredients.map(i => i.value));
    const instructionsJson = JSON.stringify(data.instructions.map(i => i.value));
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-16">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        <div className="space-y-8">
          <div>
            <label className="block text-[10px] font-label font-bold tracking-widest text-on-surface-variant uppercase mb-2">Titel</label>
            <input
              {...register("title")}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <label className="block text-[10px] font-label font-bold tracking-widest text-on-surface-variant uppercase mb-2">Collection</label>
              <select
                {...register("category")}
                className="w-full bg-surface-container-high border-none rounded-lg px-4 py-4 text-sm font-label focus:ring-2 focus:ring-primary/40 focus:outline-none"
              >
            ...
            <div>
              <label className="block text-[10px] font-label font-bold tracking-widest text-on-surface-variant uppercase mb-2">Ernährungsform</label>
              <select
                {...register("dietary")}
                className="w-full bg-surface-container-high border-none rounded-lg px-4 py-4 text-sm font-label focus:ring-2 focus:ring-primary/40 focus:outline-none"
              >
